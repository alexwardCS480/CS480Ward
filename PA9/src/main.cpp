...
void render()
{
    ...
    string playerTwoStr = "Player Two Score:" + numToStr(scoreTwo);
    ...
    renderBitmapString(30,60,(void *)GLUT_BITMAP_HELVETICA_18,playerTwoStr);
    glPushMatrix();
    //glMatrixMode(GL_PROJECTION);
    glPopMatrix();
    glMatrixMode(GL_MODELVIEW);
    ...
    switch(viewNum)
        {
            ...
            case 1:
                view = glm::lookAt( glm::vec3(tempPlayerOne.x, 40.0, tempPlayerOne.z), //Eye Position
                        glm::vec3(tempPlayerOne.x, tempPlayerOne.y, tempPlayerOne.z), //Focus point
                        glm::vec3(0.0, 0.0, 1.0)); //Positive Y is up
                break;
...
bool initialize()    
{
    ...
    globalObjCount++;
    Vertex *geometry;
    loadedSuccess = loadOBJ("iceRink.obj", &geometry, 0);
    if ( !loadedSuccess )
    {
    ...
    objectsDataList.addObject(0, tempShape, tempMotionState, tempMass, tempInertia, vbo_geometry, numberTriangles, 3.5f, tempRigidBody, textureFileName);
    ...
    objTriMesh = new btTriangleMesh();
    loadedSuccess = loadOBJ("paddleOne.obj", &cylinder, 1);
    if ( !loadedSuccess )
    {
        cout << "OBJ file not found or invalid format" << endl;
        return false;
    ...
    objectsDataList.addObject(1, tempShape, tempMotionState, tempMass, tempInertia, vbo_cylinder, numberTriangles, 1.5f, tempRigidBody, textureFileName);
            ...
            if (objectsDataList.getRigidBody(index) && objectsDataList.getRigidBody(index)->getMotionState())
                {
                     ...
                     if ( index == 1 )
                        ...
                        {
                         startPos = btVector3(0.0f,-2.0f,0.0f);
                        }
